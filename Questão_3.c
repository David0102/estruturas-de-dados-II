...
Arvbin *cria_no_arv(char pal_port[100], char pal_ing[100]);
Lista *cria_no_list(char str[100]);
void inseri_lista(Lista **lista, Lista *NO);
int inserir_arv(Arvbin **raiz, Arvbin *no);
int ordena(char str1[100], char str2[100]);
void imprimir_palavras_uni(Arvbin *raiz);
void imprimir_lista(Lista *lista);
void ocorrencia(Lista *raiz, Arvbin *no, int *oc);
Arvbin *busca(Arvbin *raiz, char palavra[100]);
int remover1(Arvbin **raiz, char palavra[100]);
...
Arvbin *cria_no_arv(char pal_port[100], char pal_ing[100]){//cria o no da arvore
    Lista *No, *lista;
    lista = NULL;

    Arvbin *no = (Arvbin*)malloc(sizeof(Arvbin));//parte de alocar o no da arvore
...
int inserir_arv(Arvbin **raiz, Arvbin *no){//insere na arvore
    Lista *NO;
    int inseriu = 0, oc;
    if(*raiz == NULL){
...
void imprimir_palavras_uni(Arvbin *raiz){//funcao de imprimir as palavras em portuques e suas traducoes  
...
Arvbin *busca(Arvbin *raiz, char palavra[100]){//funcao de buscar uma palavra na arvore
    Arvbin *ret;
    if(raiz == NULL){
...
int remover1(Arvbin **raiz, char palavra[100]){//funcao para remover uma palavra a suas equivalencias em ingles
    int removeu = 0;
    if(*raiz == NULL){
            ...
            if((*raiz)->Esq == NULL && (*raiz)->Dir == NULL){//remove no folha
				...
				*raiz = NULL;
				...
				if((*raiz)->Esq != NULL && (*raiz)->Dir != NULL){
					Arvbin *aux = (*raiz)->Esq;//recebe a esquerda, entao procurado o maior no da direita da esquerda
                    Lista *aux1;
...
void libera_arv(Arvbin *raiz){//funcao para liberar a memoria alocada de de uma arvore
